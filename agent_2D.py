...
from replay_buffer import ReplayBuffer2D
...
class Agent:
    def __init__(self, alpha, beta, input_dims, n_actions, env_high, env_low, tau, latent_dim, auto_encoder_path, gamma=0.99,
        ...
        self.memory = ReplayBuffer2D(latent_dim + sensors_size, n_actions, max_size, input_shape=input_dims)
        self.batch_size = batch_size
        self.warmup = warmup
        self.noise = noise
        self.learn_step_cntr = 0
        self.step_cntr = 0
        self.auto_encoder_path = auto_encoder_path

        self.auto_encoder = AutoEncoder(layer1_size, layer2_size, latent_dim, input_dims)
        self.auto_encoder.compile(optimizer=Adam(learning_rate=beta), loss='mean_squared_error')
    ...
    def remember(self, state, action, reward, state_, done, state_raw):
        self.memory.save_step(state, action, reward, state_, done, state_raw)
